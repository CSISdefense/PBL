...
PBLgraph<-PBLgraph+
    theme(legend.position="bottom",axis.text.x = element_text(angle = 90, hjust = 1))+
    scale_x_date(labels=date_format("'%y"), limits=c(as.Date("2000-01-01"),as.Date("2014-01-01")),breaks=date_breaks("year"))
...
PBLgraph<-PBLgraph+
    theme(legend.position="bottom",axis.text.x = element_text(angle = 90, hjust = 1))+
    scale_x_date(labels=date_format("'%y"), limits=c(as.Date("2005-01-01"),as.Date("2014-01-01")),breaks=date_breaks("year"))
...
for(m in unique(DLApblScore$MajorCommandName)){
    ...
    MCCpblScore$Graph[is.na(MCCpblScore$ContractingOfficeName)]<-FALSE
    ...
    png(
      paste("Output\\",
               ...
               PBLgraph<-LatticePlotWrapper(
                   "Addressable Market Category"
                   ,paste(m,"Contract Spending by KO \nand Addressable Market Category (No History Excluded)")
                   ,"Fiscal Year"
                   ,"Contract Obligations, 2014 Billions"
                   ,Coloration
                   ,MCCpblScore
...
PBLscoreTable<-ddply(PBLscore, c("Fiscal.Year"
                                 ,"Customer"
                                 ,"SubCustomer"
                                 ,"AddressCat"
...
DLAscoreTable<-subset(PBLscoreTable,SubCustomer=="DLA" & Fiscal.Year>=as.Date("2013-09-30"))

View(DLAscoreTable)


write.csv(DLAscoreTable,paste(paste("Output\\DLA_addressable"
                                    ,"Obligation_2014"
                                    ,"ProductOrServiceCode"
                                    ,"2012-2014"
                                    ,sep="_"
